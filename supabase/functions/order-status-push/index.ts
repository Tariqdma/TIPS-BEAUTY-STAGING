...
Deno.serve(async (request) => {
  ...
  try {
    ...
    // Delivery sends the operational confirmation plus one distinct review invitation.
    ...
    const expoResponse = await fetch("https://exp.host/--/api/v2/push/send", {
      ...
      body: JSON.stringify(targets.map(({ token, eventType }) => {
        ...
        return { to: token.expo_push_token, sound: "default", title: message.title, body: message.body(order.order_number ?? ""), channelId: "orders", data: { orderId: order.id, url: message.url, eventType } };
      })),
    });
    ...
    const logs = targets.map(({ token, eventType }, index) => {
      ...
      return { order_id: order.id, customer_id: order.customer_id, push_token_id: token.id, event_type: eventType, status: ticket.status === "ok" ? "submitted" : "failed", expo_ticket_id: ticket.id ?? null, error_message: ticket.message ?? null, response_payload: ticket };
    });
